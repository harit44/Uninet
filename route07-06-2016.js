...
var serviceac = function(req, res, next) {
	if(!req.isAuthenticated()) {
		...
		if (user.role === 1) {
          ...
          var query = connection.query('SELECT ResourceAllocated.resourceString1,ResourceAllocated.resourceString2,ResourceAllocated.IP1,ResourceAllocated.IP2,ResourceAllocated.startTime,ResourceAllocated.endTime FROM ResourceAllocated LEFT JOIN ServiceActivities ON ResourceAllocated.said=ServiceActivities.said JOIN ServiceRequests ON ServiceActivities.sid=ServiceRequests.sid WHERE user = ? ',[user.id],function(err,rows2){
					  res.render('serviceac',{page_title:"user", user:user, data:rows, history:rows2});
				  });
        });
			});
		}
...
var ccServiceac = function(req,res){
  ...
  if(!req.isAuthenticated()) {
    ...
    req.getConnection(function (err, connection) {
      console.log("555555555555555555555555555555555555555555555");
      var query = connection.query("UPDATE ServiceActivities INNER JOIN ResourceAllocated ON ServiceActivities.said = ResourceAllocated.said SET actbyuser=1 WHERE ServiceActivities.said = ?",[id], function(err, rows){ 
        if(err)
				...
				var emailtemp = connection.query(queryString, function(err, result){ //Query data from database
					if(!err) {
						...
						var logdata = {
								...
								emailData  	: temp[0]+Nametemp+temp[1]+Userid+temp[2]+Userpassword+temp[3]
						};
			...
			req.getConnection(function(err,connection){
				...
				var query = connection.query('SELECT * FROM Emailtext',function(err,rows){
					if(!err){
						...
						var query = connection.query('SELECT * FROM EmailLogs',function(err,logs){
							if(!err){
...
var servicemanage = function(req, res, next) {
	if(!req.isAuthenticated()) {
		...
		if (user.role !== 1) {
			...
			req.getConnection(function(err,connection){
        var query = connection.query('SELECT * FROM ResourceAllocated LEFT JOIN ServiceActivities ON ResourceAllocated.said=ServiceActivities.said JOIN ServiceRequests ON ServiceActivities.sid=ServiceRequests.sid JOIN User ON User.id = ServiceRequests.user',function(err,history){
					...
					var query = connection.query('SELECT * FROM ActivePackage',function(err,active){
						var query = connection.query('SELECT * FROM Netfpga_Status',function(err,rows)
						{
							...
							var query = connection.query('SELECT *  FROM Nagios_Status',function(err,nagios){
								...
								res.render('servicemanage',{page_title:"servicemanage",active:active,request:request,user: user,nagios:nagios,data:rows,history:history});
					...
					var query = connection.query("INSERT INTO ActivePackage SET ?",data, function(err,rows)
					{
						connection.query("UPDATE ServiceActivities SET actType = 1 WHERE said = ? ",[id], function(err,rows){  
							...
							var emailtemp = connection.query(queryString, function(err, result){ //Query data from database
								if(!err) {
									//connection.release();
									var temp = result[1].Text.split("*").map(function (val) {
										return (val);
									});
